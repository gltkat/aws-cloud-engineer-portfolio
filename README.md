...
- Terraformのリソースは依存関係がどうなっているか把握してリソースを規定していく必要がある

- ASGによってターゲットグループへのインスタンス増減が行われる
- 増減されるインスタンスの内容はLaunch Templateによって規定される
- インスタンスへ付与されるIAMはInstance Profileによって指定される
- Instance ProfileにアタッチするIAMロールは１つに限られる
- IAMロールには複数のポリシーアタッチすることができる
- AWSではIMAロール中のポリシーを抜き出して利用することが難しい
- 
-
...
Terraformを使ったASGの導入ではCloudFormationでの体験のようにASGのスケーリング方法の違いなどを確認する内容が想定されていた。

しかし構築する過程においてリソース間の依存関係という問題をハッキリ意識して構築することが主眼となっていった。

リスナー・ALB・ターゲトグループの依存関係についてはALBを構築する段階で意識されていたが
ASGをリソースとして構築する段階で初めてターゲットグループがALBとASGのどちら側に寄ったリソースとなるのか意識することが出来た。
