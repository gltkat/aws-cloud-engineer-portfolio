...
Webサーバー導入後にNAT Gatewayを削除しても、
既にWebサーバーが起動済みであれば
ALBヘルスチェックは正常を維持することを確認できた。

この構成での実装初期においてはAWSコンソールからEC2の停止・削除・再作成などを繰り返していたため、Terraform管理外の変更が多数発生していた。

その結果、Terraform Stateと実環境の整合性が崩れていた可能性があり、原因の切り分けが困難になった。


しかしそのための検証に多大な時間を費やしてしまった。

後日、Terraformのみで環境構築とNAT Gateway削除を実施したところ、理論通り
